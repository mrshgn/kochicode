...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    this.gl = this.canvas.getContext('webgl2', { antialias: true, alpha: true }); // Make background transparent
    const gl = this.gl;
    if (!gl) {
  ...
  #initTexture() {
    ...
    // Initialize with a 1x1 transparent pixel so the texture is "complete" and doesn't render as black while loading
        ...
        item =>
          new Promise(resolve => {
            ...
            img.onerror = () => {
                ...
                tempCtx.fillStyle = '#333'; // Dark gray fallback
...
// Portfolio items for the KochiCode site
const portfolioItems = [
  {
    image: 'assets/snap_tharians.png',
    link: 'https://tharians.azamon.xyz/',
    title: 'Tharians',
    description: 'Immersive brand experience & digital storefront.'
  },
  {
    image: 'assets/snap_jwellery.png',
    link: 'https://jwellery.azamon.xyz/',
    title: 'Jewellery Showcase',
    description: 'High-fidelity eCommerce interface with fluid interactions.'
  },
  {
    image: 'assets/snap_providence.png',
    link: 'https://providence.azamon.xyz/',
    title: 'Providence',
    description: 'Enterprise-grade web application architecture.'
  }
];
...
function initInfiniteMenu() {
    ...
    const actionEl = document.getElementById('im-action');
    ...
    const handleMovementChange = (isMoving) => {
        if (isMoving) {
            if (titleEl) { titleEl.classList.remove('active'); titleEl.classList.add('inactive'); }
            if (descEl) { descEl.classList.remove('active'); descEl.classList.add('inactive'); }
            if (actionEl) { actionEl.classList.remove('active'); actionEl.classList.add('inactive'); }
        } else {
            if (titleEl) { titleEl.classList.remove('inactive'); titleEl.classList.add('active'); }
            if (descEl) { descEl.classList.remove('inactive'); descEl.classList.add('active'); }
            if (actionEl) { actionEl.classList.remove('inactive'); actionEl.classList.add('active'); }
        }
    };

    let sketch = new InfiniteGridMenu(
        canvas,
        portfolioItems,
    ...
    if (actionEl) {
        actionEl.addEventListener('click', () => {
            if (activeItemData && activeItemData.link) {
                window.open(activeItemData.link, '_blank');
            }
        });
    }
}
